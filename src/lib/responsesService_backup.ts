...
class ResponsesDatabase {
  ...
  private initializeMockData() {
    ...
    const mockResponses: Response[] = [
      ...
      },
      {
        ...
        taskId: 'task-2',
        volunteerId: 'vol-3',
        volunteerName: 'Сания Жусупова',
        volunteerEmail: 'sania@example.com',
        volunteerAvatar: '/avatars/sania.jpg',
        status: 'rejected',
        message: 'Студентка, хочу получить опыт в социальных проектах.',
        appliedAt: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000), // 5 дней назад
        reviewedAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000), // 2 дня назад
        reviewedBy: 'org-1',
        reviewMessage: 'К сожалению, не подходит по требованиям к опыту.',
        matchingScore: 0.65,
        hardSkills: ['Обучение'],
        softSkills: ['Эмпатия', 'Терпение'],
        experience: 'Начинающий волонтер',
        availability: 'weekends',
        motivation: 'Хочу помогать детям и развиваться'
      }
    ];
    ...
    const mockNotifications: ResponseNotification[] = [
      ...
      },
      {
        ...
        recipientId: 'org-1',
        recipientRole: 'organizer',
        message: 'Дмитрий Петров принял приглашение на задачу "Уборка центрального парка"',
  ...
  createResponse(responseData: Omit<Response, 'id' | 'appliedAt'>): Response {
    ...
    this.createNotification({
      ...
      organizerId: 'org-1', // В реальном приложении здесь будет ID организатора
  ...
  updateResponseStatus(responseId: string, status: Response['status'], reviewMessage?: string, organizerId?: string): Response | null {
    ...
    if (oldStatus === 'pending' && (status === 'accepted' || status === 'rejected')) {
      this.createNotification({
        type: status === 'accepted' ? 'response_accepted' : 'response_rejected',
        taskId: this.responses[responseIndex].taskId,
        taskTitle: 'Задача', // В реальном приложении здесь будет название задачи
        volunteerId: this.responses[responseIndex].volunteerId,
        volunteerName: this.responses[responseIndex].volunteerName,
        organizerId: organizerId || 'org-1',
        message: reviewMessage || `Ваш отклик был ${status === 'accepted' ? 'принят' : 'отклонен'}`
      });
    }
  ...
  // Получить статистику откликов для задачи
  getTaskResponseStats(taskId: string): ResponseStats {
    ...
    return {
      ...
      withdrawn: taskResponses.filter(r => r.status === 'withdrawn').length
    };
  }

  // Отфильтровать отклики
  filterResponses(responses: Response[], filters: ResponseFilters): Response[] {
  ...
  getNotificationsForOrganizer(organizerId: string): ResponseNotification[] {
    return this.notifications
      .filter(notif => notif.organizerId === organizerId)
  ...
  getUnreadCount(organizerId: string): number {
    return this.notifications
      .filter(notif => notif.organizerId === organizerId && !notif.read)
      .length;
  }
}
