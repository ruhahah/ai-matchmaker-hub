...
export default function AITaskCoordinator() {
  ...
  const generateQuestions = (userInput: string): AIQuestion[] => {
    ...
    if (userInput.toLowerCase().includes('спорт') || userInput.toLowerCase().includes('физическая')) {
      questions.push({
        id: 'physical_level',
        question: 'Какой уровень физической подготовки требуется?',
        type: 'select',
        options: ['Низкий (без нагрузки)', 'Средний (легкая активность)', 'Высокий (интенсивная нагрузка)', 'Профессиональный'],
        required: true,
        context: 'Физическая нагрузка влияет на требования к волонтерам'
      });
    }

    if (userInput.toLowerCase().includes('компьютер') || userInput.toLowerCase().includes('it')) {
      questions.push({
        id: 'tech_level',
        question: 'Какой уровень технических навыков необходим?',
        type: 'select',
        options: ['Базовые навыки', 'Средний уровень', 'Продвинутый пользователь', 'Профессиональные навыки'],
        required: true,
        context: 'Технические требования для выполнения задачи'
      });
    }
    ...
    // Дополнительные вопросы для сложных задач
    if (userInput.length > 100 || userInput.toLowerCase().includes('организовать') || userInput.toLowerCase().includes('провести')) {
      questions.push({
        id: 'equipment',
        question: 'Какое оборудование или материалы потребуются?',
        type: 'text',
        required: false,
        context: 'Оборудование влияет на подготовку и бюджет задачи'
      });

      questions.push({
        id: 'special_skills',
        question: 'Требуются ли специальные навыки или квалификация?',
        type: 'multiselect',
        options: ['Медицинские знания', 'Педагогические навыки', 'Технические навыки', 'Юридические знания', 'Иностранные языки', 'Водительские права', 'Другое'],
        required: false,
        context: 'Специальные навыки помогают найти подходящих волонтеров'
      });
    }
  ...
  const generateDetailedDescription = (answers: Record<string, any>): string => {
    ...
    if (answers.physical_level) {
      desc += `Уровень физической нагрузки: ${answers.physical_level}. `;
    }
    
    if (answers.tech_level) {
      desc += `Требования к техническим навыкам: ${answers.tech_level}. `;
    }
    
    if (answers.special_skills && answers.special_skills.length > 0) {
      desc += `Специальные навыки: ${answers.special_skills.join(', ')}. `;
    }
  ...
  const generateSkills = (answers: Record<string, any>): string[] => {
    ...
    if (answers.special_skills && Array.isArray(answers.special_skills)) {
      skills.push(...answers.special_skills);
    }
  ...
  const generateRequirements = (answers: Record<string, any>): string[] => {
    ...
    if (answers.physical_level && answers.physical_level !== 'Низкий') {
      requirements.push('Хорошая физическая подготовка');
    }
  ...
  const generateEquipment = (answers: Record<string, any>): string[] => {
    ...
    if (answers.category?.includes('IT')) {
      equipment.push('Ноутбук', 'Зарядное устройство');
    }
